...
class AdaptiveJoinTest extends TableTestBase {
  ...
  def before(): Unit = {
    ...
    util.tableConfig.set(OptimizerConfigOptions.TABLE_OPTIMIZER_ADAPTIVE_BROADCAST_JOIN_STRATEGY, 
  ...
  def testWithStaticBroadcastJoin(): Unit = {
    util.tableEnv.getConfig.set(ExecutionConfigOptions.TABLE_EXEC_DISABLED_OPERATORS,
        "SortMergeJoin,ShuffleHashJoin,NestedLoopJoin")
    util.tableEnv.getConfig.set(OptimizerConfigOptions.TABLE_OPTIMIZER_BROADCAST_JOIN_THRESHOLD, 
  ...
  def testWithBroadcastJoinRuntimeOnly(): Unit = {
    ...
    util.tableEnv.getConfig.set(OptimizerConfigOptions.TABLE_OPTIMIZER_BROADCAST_JOIN_THRESHOLD,
      Long.box(Long.MaxValue))
    util.tableEnv.getConfig.set(OptimizerConfigOptions.TABLE_OPTIMIZER_ADAPTIVE_BROADCAST_JOIN_STRATEGY,
  ...
  def testShuffleJoinWithForwardForConsecutiveHash(): Unit = {
    util.tableEnv.getConfig.set(OptimizerConfigOptions.TABLE_OPTIMIZER_MULTIPLE_INPUT_ENABLED,
      Boolean.box(false))
    val sql =
      """
        |WITH
  ...
  @Test
  def testWithMultipleInput(): Unit = {
    val sql =
      """
        |SELECT * FROM
