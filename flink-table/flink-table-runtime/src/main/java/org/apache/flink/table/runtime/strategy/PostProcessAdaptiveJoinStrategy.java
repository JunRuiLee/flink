...
/**
 * The post-processing phase of adaptive join optimization, which must be placed at the end of 
 * all adaptive join optimization strategies.
 */
public class PostProcessAdaptiveJoinStrategy extends BaseAdaptiveJoinOperatorOptimizationStrategy {
    ...
    public void tryOptimizeAdaptiveJoin(
        ...
        if (context.areAllUpstreamNodesFinished(adaptiveJoinNode)) {
            ...
            if (!adaptiveJoin.isLeftBuild()) {
                ...
                context.modifyStreamEdge(streamEdgeUpdateRequestInfos);
                context.modifyStreamNode(generateStreamNodeUpdateRequestInfos(adaptiveJoinNode));
            }
    ...
    private List<StreamNodeUpdateRequestInfo> generateStreamNodeUpdateRequestInfos(
        ...
        Preconditions.checkState(
                ...
                String.format(
                        "Adaptive join node currently only supports two "
