...
public class PlannerCodeAccessor {
    ...
    static {
        ...
        KNOWN_MODULE_ASSOCIATIONS.put("org.apache.flink.table.planner.adaptive", "flink-table-runtime");
    }

    private final ClassLoader submoduleClassLoader;

    private PlannerCodeAccessor() {
        try {
            final ClassLoader flinkClassLoader = PlannerCodeAccessor.class.getClassLoader();
            final Path tmpDirectory =
                    Paths.get(ConfigurationUtils.parseTempDirectories(new Configuration())[0]);
            Files.createDirectories(tmpDirectory);

            Path plannerJar = extractResource(flinkClassLoader, tmpDirectory);

            this.submoduleClassLoader =
                    new ComponentClassLoader(
                            new URL[] {plannerJar.toUri().toURL()},
                            flinkClassLoader,
                            OWNER_CLASSPATH,
                            COMPONENT_CLASSPATH,
    ...
    private Path extractResource(ClassLoader flinkClassLoader, Path tmpDirectory)
            throws IOException {
        String[] splitName = PlannerCodeAccessor.FLINK_TABLE_PLANNER_FAT_JAR.split("\\.");
        ...
        if (resourceStream == null) {
            throw new RuntimeException(
                    "Flink table planner could not be found.\n"
                            + "If you're running a test, please add the following dependency"
                            + " to your pom.xml.\n<dependency>\n   "
    ...
    private static class CodegenLoaderHolder {
        private static final PlannerCodeAccessor INSTANCE = new PlannerCodeAccessor();
    }

    public static PlannerCodeAccessor getInstance() {
        return PlannerCodeAccessor.CodegenLoaderHolder.INSTANCE;
    }
}
