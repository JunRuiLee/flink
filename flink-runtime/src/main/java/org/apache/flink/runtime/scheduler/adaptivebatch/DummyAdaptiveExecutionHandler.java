...
public class DummyAdaptiveExecutionHandler implements AdaptiveExecutionHandler {
    ...
    public void updateForwardGroupByNewlyParallelism(
            ...
            int newParallelism,
            BiConsumer<JobVertexID, Integer> executionJobVertexParallelismUpdater) {
        ForwardGroup forwardGroup = forwardGroupsByJobVertexId.get(jobVertexId);

        if (forwardGroup != null && !forwardGroup.isParallelismDecided()) {
            forwardGroup.setParallelism(newParallelism);

            for (JobVertexID id : forwardGroup.getJobVertexIds()) {
                executionJobVertexParallelismUpdater.accept(id, newParallelism);
            }
        }
    }
}
