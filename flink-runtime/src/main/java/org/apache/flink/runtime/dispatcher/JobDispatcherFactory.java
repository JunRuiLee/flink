...
public enum JobDispatcherFactory implements DispatcherFactory {
    ...
    public MiniDispatcher createDispatcher(
            ...
            PartialDispatcherServicesWithJobPersistenceComponents
                    partialDispatcherServicesWithJobPersistenceComponents)
            throws Exception {
        final StreamGraph recoveredStreamGraph = Iterables.getOnlyElement(recoveredJobs, null);
        final JobResult recoveredDirtyJob =
                Iterables.getOnlyElement(recoveredDirtyJobResults, null);

        Preconditions.checkArgument(
                recoveredStreamGraph == null ^ recoveredDirtyJob == null,
                "Either the StreamGraph or the recovered JobResult needs to be specified.");
