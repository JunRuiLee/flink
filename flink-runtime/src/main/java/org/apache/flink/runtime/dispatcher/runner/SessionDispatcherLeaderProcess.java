...
public class SessionDispatcherLeaderProcess extends AbstractDispatcherLeaderProcess
    ...
    private void createDispatcherIfRunning(
        ...
        runIfStateIs(
                State.RUNNING,
                () -> createDispatcher(executionPlans, recoveredDirtyJobResults));
    }
    ...
    private Optional<CompletableFuture<Void>> submitAddedJobIfRunning(
    ...
    private CompletableFuture<Void> submitAddedJob(ExecutionPlan executionPlan) {
        ...
        if (executionPlan.hasJobGraph()) {
            return dispatcherGateway
                    .submitJob(executionPlan.getJobGraph(), RpcUtils.INF_TIMEOUT)
                    .thenApply(FunctionUtils.nullFn())
                    .exceptionally(this::filterOutDuplicateJobSubmissionException);
        } else {
            return dispatcherGateway
                    .submitJob(
                            executionPlan.getSerializedStreamGraph(),
